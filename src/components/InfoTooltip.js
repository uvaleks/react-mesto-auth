...
function InfoTooltip({ tooltipStatus, isOpen, onClose }) {
  return (
    <div className={`popup ${isOpen ? 'popup_opened' : ''}`}>
          <div className="tooltip">
              <img className="tooltip__status-icon" src={(tooltipStatus === 'ok') ? okTooltipStatus : errorTooltipStatus} />
              <h2 className="tooltip__title">{(tooltipStatus === 'ok') ? "Вы успешно зарегистрировались!" : `Что-то пошло не так! Попробуйте ещё раз.`}</h2>
